...
public class ForwardLinked<T> implements Iterable<T> {
    ...
    public void addFirst(T value) {
        Node<T> node = new Node<>(value, head);    // создаём новый узел, в который записываем значение value и ссылку на предыдущий элемент;
    ...
    public boolean revert() {
        ...
        head.next = null; // предыдущий = null; //
        next = head;  // текущий = голова;  //
        if (next == null) {   // если текущий узел = null, то выходим из программы  //
            return false;
        }
        next.next = head.next;    // следующий = текущий.следующий // 2 = 1.следующий
        next.next = head;    // текущий.следующий = предыдущий; // 2.следующий = 1
        head = next;         // предыдущий = текущий;  //    1 = текущий;
        next = head.next;    // текущий = следующий;  //     2 = следующий;
        head = next;         // голова = предыдущий; //      голова = 2;
        return next == head;
    }
